...
def check_alerts():
    ...
    with transaction.atomic():
        ...
        for row in inventory_overview():
            if row['low_stock']: emit('LOW_STOCK',Item.objects.get(pk=row['id']),'物料库存偏低',f"{row['name']} · 可领 {row['available']} {row['base_uom']} / 阈值 {row['reorder_qty']}",recipients,str(today))
        for b in Batch.objects.filter(expires_on__gte=today,expires_on__lte=today+timedelta(days=30),balances__on_hand_qty__gt=0).distinct():
            emit('EXPIRING',b,'批次即将到期',f'{b.batch_no} · {b.expires_on}',recipients,str(today))
        for t in Task.objects.filter(due_date__lt=today).exclude(status__in=['DONE','CANCELLED']).select_related('project'):
            emit('OVERDUE_TASK',t,'任务已逾期',t.title,[t.assignee_id,t.project.owner_id],str(today))
...
def consume_events(limit=100):
    ...
    for _ in range(limit):
        ...
        try:
            ...
            with transaction.atomic():
                e=OutboxEvent.objects.select_for_update().get(pk=eid)
                e.attempts+=1; e.locked_until=None; e.last_error='通知投递失败，请检查后台日志'
...
def import_preview(user,kind,mode,name,raw,mapping,key,rid):
    allow(user,'ADMIN')
    require(kind in MODELS and mode in ['CREATE','UPDATE'],'INVALID_IMPORT','请选择有效实体与导入模式')
    require(len(raw)<=5*1024*1024,'FILE_TOO_LARGE','CSV 文件最大 5 MB',400)
    sha=hashlib.sha256(raw).hexdigest()
    key=text(key or '', 'Idempotency-Key',128)
    old=ImportJob.objects.filter(idempotency_key=key).first()
    if old:
        require(old.file_sha256==sha and old.entity_type==kind and old.mode==mode,'IDEMPOTENCY_CONFLICT','同一幂等键用于不同导入',409)
        return old
    try: decoded=raw.decode('utf-8-sig')
    except UnicodeDecodeError: fail('INVALID_ENCODING','请上传 UTF-8 编码 CSV',400)
    try:
        reader=csv.DictReader(io.StringIO(decoded),strict=True)
        require(bool(reader.fieldnames),'INVALID_CSV','CSV 缺少列标题',400)
        require(len(set(reader.fieldnames))==len(reader.fieldnames),'INVALID_CSV','CSV 存在重复列标题',400)
        inputs=[]
        for row in reader:
            require(None not in row and all(v is not None for v in row.values()),'INVALID_CSV','CSV 列数与标题不匹配',400)
            inputs.append(row)
            require(len(inputs)<=5000,'TOO_MANY_ROWS','CSV 最多 5000 行',400)
    except csv.Error: fail('INVALID_CSV','CSV 格式错误',400)
    require(bool(inputs),'EMPTY_CSV','CSV 没有数据行',400)
    ...
    for n,row in enumerate(inputs,1):
        ...
        try:
            require(counts[codes[n-1]]==1,'DUPLICATE_CODE','同一文件存在重复编码',422,'code')
            fields=normalized(kind,mapped)
            existing=MODELS[kind].objects.filter(code=fields['code']).first()
            if mode=='CREATE': require(existing is None,'DUPLICATE_CODE','CREATE 模式下编码已存在',409,'code')
            else:
                require(existing is not None,'NOT_FOUND','UPDATE 模式下编码不存在',422,'code')
...
def execute_import(user,id,rid):
    ...
    require(job.status in ['READY','RUNNING','COMPLETED','PARTIAL_FAILED','FAILED'],'IMPORT_NOT_READY','只有预检全部通过的作业可以执行')
        ...
        try:
            with transaction.atomic():
                ...
                if job.mode=='UPDATE':
                    ...
                    require(target is not None,'NOT_FOUND','目标记录已不存在')
            ...
            with transaction.atomic():
                ...
                if row.status=='SUCCEEDED': continue
                row.status='FAILED'; row.errors_json=[{'field':getattr(e,'field',''),'code':getattr(e,'code','INVALID_ROW'),'message':getattr(e,'message','行数据不合法')}]; row.save()
    with transaction.atomic():
        RuntimeState.objects.select_for_update().get(pk=1)
...
def queue_import(user,id,rid):
    allow(user,'ADMIN');job=obj(ImportJob,id)
    require(job.status in ['READY','RUNNING','COMPLETED','PARTIAL_FAILED','FAILED'],'IMPORT_NOT_READY','请先修正预检错误后重新上传')
        ...
        try:execute_import(job.created_by,job.id,'import-worker-'+str(job.id))
            ...
            with transaction.atomic():
                ...
                for row in job.rows.exclude(status='SUCCEEDED'):
                    row.status='FAILED';row.errors_json=[{'field':'','code':'FORBIDDEN','message':'作业创建者的账号或管理员角色已失效'}];row.save()
